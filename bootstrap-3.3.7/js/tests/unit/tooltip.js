$(function () {
  ...
  QUnit.test('should correctly position tooltips on SVG elements', function (assert) {
    ...
    var styles = '<style>'
        ...
        + '.tooltip .tooltip-inner { width: 24px; height: 24px; fonts-family: Helvetica; }'
        + '</style>'
    var $styles = $(styles).appendTo('head')
  ...
  QUnit.test('should correctly determine auto placement based on container rather than parent', function (assert) {
    ...
    var styles = '<style>'
        + '.tooltip, .tooltip *, .tooltip *:before, .tooltip *:after { box-sizing: border-box; }'
        + '.tooltip { position: absolute; display: block; fonts-size: 12px; line-height: 1.4; }'
        + '.tooltip .tooltip-inner { max-width: 200px; padding: 3px 8px; fonts-family: Helvetica; text-align: center; }'
        + '#trigger-parent {'
        + '  position: fixed;'
        + '  top: 100px;'
        + '  right: 17px;'
